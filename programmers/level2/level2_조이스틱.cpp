...
int solution(string name) {
    ...
    while(true){   
        ...
        while(initial_name[left_index] == name[left_index]){//문자가 같을 경우에는, 알파벳을 바꿀 필요가없으므로 커서를 계속 이동시킨다.
            left_index--;
            left_cnt++;
        ...
        while(initial_name[right_index] == name[right_index]){//문자가 같을 경우에는, 알파벳을 바꿀 필요가없으므로 커서를 계속 이동시킨다.
            right_index++;
            right_cnt++;
        ...
        if (left_cnt < right_cnt){// 왼쪽이 최소일경우
            answer += left_cnt;
